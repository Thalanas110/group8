...
export function TeacherExams() {
  ...
  const handleSave = () => {
    if (!form.title.trim() || !form.classId) { toast.error('Please fill in all required fields'); return; }
    if (form.questions.length === 0) { toast.error('Add at least one question'); return; }
    if (editingExam) {
      updateExam(editingExam.id, { ...form, teacherId: currentUser.id });
      toast.success('Exam updated successfully');
    } else {
      addExam({ ...form, teacherId: currentUser.id });
  ...
  const toggleStatus = (exam: Exam) => {
    ...
    updateExam(exam.id, { status: next });
